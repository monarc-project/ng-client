(function () {
  ...
  function ClientRecommendationService($resource, $rootScope, MassDeleteService, gettextCatalog) {
    ...
    var attachToRisk = function (rec_id, risk_id, op, success, error) {
      let data = {recommendation: rec_id}
      if (op) {
        data['instanceRiskOp'] = risk_id;
      } else {
        data['instanceRisk'] = risk_id;
      }
      new self.ClientRecommendationRiskResource(data).$save(success, error);
    };

    var detachFromRisk = function (risk_id, success, error) {
      self.ClientRecommendationRiskResource.delete({id: risk_id}, success, error);
    }
    ...
    var getRiskRecommendations = function (risk_id, op, success, error) {
      ...
      return self.ClientRecommendationRiskResource.query(data).$promise;
    };
